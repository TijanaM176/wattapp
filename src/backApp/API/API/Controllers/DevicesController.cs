        ...
        */
        ...
        public async Task<IActionResult> NextDaysConsumptionAndProductionTimestamps()
        {
            try
            {
                return Ok(new
                {
                    consumption = await devService.ConProdForAPeriodTimestamps(0, 1, 2),
                    production = await devService.ConProdForAPeriodTimestamps(1, 1, 2),
                });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> NextDaysConsumptionAndProduction(string id)
        {
            try
            {
                return Ok(new
                {
                    consumption = await devService.GroupedConProdForAPeriodForProsumer(id, 0, 1, 2),
                    production = await devService.GroupedConProdForAPeriodForProsumer(id, 1, 1, 2)
                });
            }catch (Exception ex)
            {
                return BadRequest(ex.Message);
