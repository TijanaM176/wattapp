...
export class RealizationChartComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    document.getElementById('modalFadeConsumptionRealizationTableBody')!.style.maxHeight = this.widthService.height * 0.6 + 'px';
    ...
    this.loadData(
      ...
      (myList: any[]) => {
        return myList.map((item) => {
          ...
          return { name: `Week ${weekNumber}`, series: item.series, date: dateMonth};
        });
      }
    );
  }
