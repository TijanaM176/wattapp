...
export class RealizationDeviceComponent implements OnInit, AfterViewInit {
  ...
  HistoryWeek(id: string) {
    ...
    this.timeService.historyDeviceWeek(this.idDev).subscribe({
      next: (response) => {
  ...
  HistoryMonth(id: string) {
    ...
    this.spiner.show();
    this.activateButton(id);
    this.timeService.historyDeviceMonth(this.idDev).subscribe({
      next: (response) => {
        const consumptionTimestamps = response.timestamps || {};
        const productionTimestamps = response.predictions || {};
  ...
  HistoryYear(id: string) {
    ...
    this.spiner.show();
    this.activateButton(id);
    this.timeService.historyDeviceYear(this.idDev).subscribe({
      next: (response) => {
        const consumptionTimestamps = response.timestamps || {};
        const productionTimestamps = response.predictions || {};
