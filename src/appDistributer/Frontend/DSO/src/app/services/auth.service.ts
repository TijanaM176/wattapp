...
export class AuthService {
  ...
  signUp(userObj: any) {
    return this.http.post<any>(
      `https://localhost:7156/api/Auth/registerProsumer`,
      userObj
    );
  }
  ...
  setUserCoordinates(coordDto: any) {
    return this.http.put<any>(
      'https://localhost:7156/api/Prosumer/SetCoordinates',
      coordDto
    );
  }
  login(loginDto: any) {
  ...
  forgotPass(email: string) {
    return this.http.post<ForgotPassword>(
      `${this.baseUrl}forgot_passwordWorker?email=` + email,
      {}
    );
  }
  ...
  resetPassword(resetPasswordObj: ResetPassword) {
    return this.http.post<any>(
      `${this.baseUrl}reset_passwordWorker`,
      resetPasswordObj
    );
  }
